...
class Zoom_Room extends Event_Location {
	...
	public function get_post( $post = null ){
		...
		if( !empty($_POST['zoom_room_id']) && preg_match('/^[a-zA-Z0-9\-_]+$/', $_POST['zoom_room_id']) ){
			$this->event->event_location_data['id'] = sanitize_text_field($_POST['zoom_room_id']);
		}
		return $return;
	}
	...
	public function validate(){
		$result = false;
		if( empty($this->event->event_location_data['id']) ){
			...
			try{
				...
				if( !empty($zoom_rooms['body']->rooms) ){
					foreach( $zoom_rooms['body']->rooms as $room ){
						if( $room->id == $this->id ){
							$result = true;
							$this->event->event_location_data['room_id'] = sanitize_text_field($room->room_id);
							$this->event->event_location_data['name'] = sanitize_text_field($room->name);
						}
					}
				}
				if( !$result ){
	...
	public function output( $what = null ){
		if( $what === null){
			return esc_html($this->event->event_location_data['name']);
		}
