...
class Zoom_Meeting extends Event_Location {
	...
	public static function init(){
		...
		add_action('em_booking_deleted', $class.'::em_booking_deleted', 10, 1);
					...
					if( !empty($zoom_response['body']) ) {
						...
						if( $this->event->event_rsvp && $this->event->get_bookings()->get_booked_spaces() > 0 ){
							foreach( $this->event->get_bookings()->get_bookings() as $EM_Booking ){
								...
								if( !empty($EM_Booking->booking_meta['zoom']) ) unset($EM_Booking->booking_meta['zoom']);
	...
	/**
	 * Helper for backwards compatibility in EM 6.2 (Atomic Tickets and Booking Meta update). This function will check the booking meta and convert it accordingly.
	 * In pre 6.2 bookings were stored in booking_meta as a assoc similar to 'zoom_meeting' => array('join_url' => 'x', 'id' => 'y')
	 * Post 6.2 this is stored as a string like _zoom_meeting_join_url, but the problem is this translates to booking_meta['zoom']['meeting_join_url']
	 * This function will get the booking meta stored, and convert it into the appropriate format.
	 *
	 * @param $EM_Booking
	 *
	 * @return array|string|false Returns 'attendees' if in attendee mode, or an array of id and join_url
	 */
	public static function get_booking_meta( $EM_Booking ){
		if( !empty($EM_Booking->booking_meta['zoom']) ){
			// we have a zoom item, deduct whether pre or post update
			$type = str_replace('zoom_', '', static::$type);
			if( $EM_Booking->booking_meta['zoom'] === 'attendees' || (!empty($EM_Booking->booking_meta['zoom'][$type]) && $EM_Booking->booking_meta['zoom'][$type] === 'attendees') ){
				return 'attendees';
			}elseif( !empty($EM_Booking->booking_meta['zoom']['id']) ){
				// new way
				return $EM_Booking->booking_meta['zoom'];
			}elseif( !empty($EM_Booking->booking_meta['zoom'][$type.'_id']) ){
				// old way, process result
				$meta = array();
				foreach( $EM_Booking->booking_meta['zoom'] as $k => $v ){
	...
	public static function em_booking_save( $result, $EM_Booking ){
		...
		if( $EM_Booking->get_event()->has_event_location(static::$type) ){
			try{
				$zoom_meta = static::get_booking_meta( $EM_Booking );
				...
				$meeting_id = $_this->id;
				if( $EM_Booking->booking_status == 1 && !$zoom_meta ){
					...
					if( $_this->supports_attendee_form() ){
						...
						if( empty($zoom_registrant['first_name']) ) $zoom_registrant['first_name'] = 'n/a'; // to prevent unnecessary errors
						if( empty($zoom_registrant['last_name']) ) $zoom_registrant['last_name'] = 'n/a'; // to prevent unnecessary errors
						...
						$zoom_meta = $zoom_client->add_registrant( $zoom_registrant, $meeting_id, static::$zoom_api_base, $auto_approve );
					}
				}elseif( $zoom_meta && $EM_Booking->booking_status !== $EM_Booking->previous_status ){
					...
					if( $zoom_meta == 'attendees' && $_this->supports_attendee_form() ){ // Pro feature
						$failed_registrations = 0;
					...
					}elseif( !empty($zoom_meta['id']) ){
						// change status of booking, nothing more
						$zoom_client->update_registrant_status( $registrant_action, $zoom_meta['id'], $EM_Booking->get_person()->user_email, $meeting_id, static::$zoom_api_base );
					}
				}
			}catch( EM_Exception $ex ){
				...
				if( method_exists( $EM_Booking, 'email_admins') ){
					$email = array(
						'admin' => array(
							'subject' => 'Booking Error - #_EVENTNAME (#_EVENTDATES)',
							'body' => "A booking for event #_EVENTNAME on #_EVENTDATES at #_EVENTTIMES could not be completed.
								\n\n Booking ID : #_BOOKINGID (#_BOOKINGADMINLINK)
								\n\n Booking Name : #_BOOKINGNAME (#_BOOKINGEMAIL)
								\n\n The following Zoom Error was returned : ". $ex->getMessage()."
								\n\n Please use the link above to edit the booking accordingly, or visit the Zoom settings - ".
								'https://zoom.us/'.static::$zoom_admin_url_base.'/'.$_this->id
						),
					);
					$EM_Booking->email_admins( $email );
				}
			...
			if( $zoom_meta ) {
				$EM_Booking->update_meta( 'zoom', $zoom_meta );
			}
		}
		return $result;
	}
		...
		if( $full_result == '#_BOOKINGZOOMJOINURL' || $full_result == '#_BOOKINGZOOMJOINLINK' ){
			if( $EM_Booking->get_event()->has_event_location(static::$type) ){
				...
				if( $_this->settings['approval_type'] == 2 ){
					...
					if ( $zoom_meta ) {
						if ($zoom_meta == 'attendees' && $_this->supports_attendee_form()) {
							...
							if (!empty($zoom_meta['join_url'])) {
								$replace = esc_url($zoom_meta['join_url']);
								if ($full_result == '#_BOOKINGZOOMJOINLINK') {
									$replace = '<a href="' . $replace . '">' . sprintf(esc_html__('Join %s', 'events-manager-zoom'), static::$zoom_name_singular) . '</a>';
								}
							}
	...
	public static function booking_admin_single( $EM_Booking ){
		...
		$zoom_meta = static::get_booking_meta( $EM_Booking );
		$supports_attendees = $zoom_meta && $zoom_meta == 'attendees' && $EM_Booking->get_event()->get_event_location()->supports_attendee_form();
	...
	public static function em_booking_deleted( $EM_Booking ){
		...
		static::em_booking_save( true, $EM_Booking );
		$EM_Booking->booking_status = false; // set back for other plugins to be none the wiser
	}
	...
	public function get_ical_location(){
		if( $this->ical_location === true ) {
			if( !empty($GLOBALS['booking_email_ical_attachments']) && $GLOBALS['booking_email_ical_attachments'] instanceof \EM_Booking ){
				$EM_Booking = $GLOBALS['booking_email_ical_attachments'];
				$zoom_meta = static::get_booking_meta($EM_Booking);
				if( !empty($zoom_meta['join_url']) ){
					return $zoom_meta['join_url'];
				}
			}
